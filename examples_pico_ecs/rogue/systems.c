...
{
    // Player turn system
    PLAYER_SYS = ecs_define_system(game->ecs, player_sys, &(ecs_sys_desc_t){ .udata = game });

    ecs_require(game->ecs, PLAYER_SYS, POS_COMP);
    ecs_require(game->ecs, PLAYER_SYS, DRAWABLE_COMP);
    ecs_require(game->ecs, PLAYER_SYS, STATS_COMP);
    ecs_require(game->ecs, PLAYER_SYS, MONSTER_COMP);

    // Monster's turns
    MONSTER_SYS = ecs_define_system(game->ecs, monster_sys, &(ecs_sys_desc_t){ .udata = game });

    ecs_require(game->ecs, MONSTER_SYS, POS_COMP);
    ecs_require(game->ecs, MONSTER_SYS, DRAWABLE_COMP);
    ecs_require(game->ecs, MONSTER_SYS, STATS_COMP);
    ecs_require(game->ecs, MONSTER_SYS, MONSTER_COMP);

    // Chest system
    CHEST_SYS = ecs_define_system(game->ecs, chest_sys, &(ecs_sys_desc_t){ .udata = game });

    ecs_require(game->ecs, CHEST_SYS, CHEST_COMP);
    ecs_require(game->ecs, CHEST_SYS, POS_COMP);
    ecs_require(game->ecs, CHEST_SYS, DRAWABLE_COMP);

    // Drawable system
    DRAWABLE_SYS = ecs_define_system(game->ecs, draw_sys, &(ecs_sys_desc_t){ .udata = game });

    ecs_require(game->ecs, DRAWABLE_SYS, POS_COMP);
    ecs_require(game->ecs, DRAWABLE_SYS, DRAWABLE_COMP);
