#pragma once
/*
    #version:1# (machine generated, don't edit!)

    Generated by sokol-shdc (https://github.com/floooh/sokol-tools)

    Cmdline:
        sokol-shdc --input particle.glsl --output particle_shader.h --format=sokol_impl --slang glsl410:hlsl5:metal_macos:metal_ios:metal_sim:glsl300es --reflection --ifdef

    Overview:
    =========
    Shader program: 'particle':
        Get shader desc: particle_shader_desc(sg_query_backend());
        Vertex Shader: vs
        Fragment Shader: fs
        Attributes:
            ATTR_particle_a_pos => 0
            ATTR_particle_a_uv => 1
            ATTR_particle_a_inst_pos => 2
            ATTR_particle_a_inst_color => 3
    Bindings:
        Uniform block 'vs_params':
            C struct: vs_params_t
            Bind slot: UB_vs_params => 0
        Texture 'u_tex':
            Image type: SG_IMAGETYPE_2D
            Sample type: SG_IMAGESAMPLETYPE_FLOAT
            Multisampled: false
            Bind slot: VIEW_u_tex => 0
        Sampler 'u_smp':
            Type: SG_SAMPLERTYPE_FILTERING
            Bind slot: SMP_u_smp => 1
*/
#include <string.h>
#if !defined(SOKOL_GFX_INCLUDED)
#error "Please include sokol_gfx.h before particle_shader.h"
#endif
#if !defined(SOKOL_SHDC_ALIGN)
#if defined(_MSC_VER)
#define SOKOL_SHDC_ALIGN(a) __declspec(align(a))
#else
#define SOKOL_SHDC_ALIGN(a) __attribute__((aligned(a)))
#endif
#endif
const sg_shader_desc* particle_shader_desc(sg_backend backend);
int particle_attr_slot(const char* attr_name);
int particle_texture_slot(const char* tex_name);
int particle_sampler_slot(const char* smp_name);
int particle_uniformblock_slot(const char* ub_name);
size_t particle_uniformblock_size(const char* ub_name);
int particle_storagebuffer_slot(const char* sbuf_name);
int particle_storageimage_slot(const char* simg_name);
int particle_uniform_offset(const char* ub_name, const char* u_name);
sg_glsl_shader_uniform particle_uniform_desc(const char* ub_name, const char* u_name);
#define ATTR_particle_a_pos (0)
#define ATTR_particle_a_uv (1)
#define ATTR_particle_a_inst_pos (2)
#define ATTR_particle_a_inst_color (3)
#define UB_vs_params (0)
#define VIEW_u_tex (0)
#define SMP_u_smp (1)
#pragma pack(push,1)
SOKOL_SHDC_ALIGN(16) typedef struct vs_params_t {
    float u_mvp[16];
} vs_params_t;
#pragma pack(pop)
#if defined(SOKOL_SHDC_IMPL)
/*
    #version 410

    uniform vec4 vs_params[4];
    layout(location = 0) in vec2 a_pos;
    layout(location = 2) in vec2 a_inst_pos;
    layout(location = 1) out vec2 uv;
    layout(location = 1) in vec2 a_uv;
    layout(location = 0) out vec4 color;
    layout(location = 3) in vec4 a_inst_color;

    void main()
    {
        gl_Position = mat4(vs_params[0], vs_params[1], vs_params[2], vs_params[3]) * vec4(a_pos + a_inst_pos, 0.0, 1.0);
        uv = a_uv;
        color = a_inst_color;
    }

*/
#if defined(SOKOL_GLCORE)
static const uint8_t vs_source_glsl410[444] = {
    ,
};
#endif
/*
    #version 410

    uniform sampler2D u_tex_u_smp;

    layout(location = 0) out vec4 frag_color;
    layout(location = 1) in vec2 uv;
    layout(location = 0) in vec4 color;

    void main()
    {
        frag_color = texture(u_tex_u_smp, uv) * color;
    }

*/
#if defined(SOKOL_GLCORE)
static const uint8_t fs_source_glsl410[227] = {
    ,
};
#endif
/*
    #version 300 es

    uniform vec4 vs_params[4];
    layout(location = 0) in vec2 a_pos;
    layout(location = 2) in vec2 a_inst_pos;
    out vec2 uv;
    layout(location = 1) in vec2 a_uv;
    out vec4 color;
    layout(location = 3) in vec4 a_inst_color;

    void main()
    {
        gl_Position = mat4(vs_params[0], vs_params[1], vs_params[2], vs_params[3]) * vec4(a_pos + a_inst_pos, 0.0, 1.0);
        uv = a_uv;
        color = a_inst_color;
    }

*/
#if defined(SOKOL_GLES3)
static const uint8_t vs_source_glsl300es[405] = {
    ,
};
#endif
/*
    #version 300 es
    precision mediump float;
    precision highp int;

    uniform highp sampler2D u_tex_u_smp;

    layout(location = 0) out highp vec4 frag_color;
    in highp vec2 uv;
    in highp vec4 color;

    void main()
    {
        frag_color = texture(u_tex_u_smp, uv) * color;
    }

*/
#if defined(SOKOL_GLES3)
static const uint8_t fs_source_glsl300es[258] = {
    ,
};
#endif
/*
    cbuffer vs_params : register(b0)
    {
        row_major float4x4 _33_u_mvp : packoffset(c0);
    };


    static float4 gl_Position;
    static float2 a_pos;
    static float2 a_inst_pos;
    static float2 uv;
    static float2 a_uv;
    static float4 color;
    static float4 a_inst_color;

    struct SPIRV_Cross_Input
    {
        float2 a_pos : TEXCOORD0;
        float2 a_uv : TEXCOORD1;
        float2 a_inst_pos : TEXCOORD2;
        float4 a_inst_color : TEXCOORD3;
    };

    struct SPIRV_Cross_Output
    {
        float4 color : TEXCOORD0;
        float2 uv : TEXCOORD1;
        float4 gl_Position : SV_Position;
    };

    void vert_main()
    {
        gl_Position = mul(float4(a_pos + a_inst_pos, 0.0f, 1.0f), _33_u_mvp);
        uv = a_uv;
        color = a_inst_color;
    }

    SPIRV_Cross_Output main(SPIRV_Cross_Input stage_input)
    {
        a_pos = stage_input.a_pos;
        a_inst_pos = stage_input.a_inst_pos;
        a_uv = stage_input.a_uv;
        a_inst_color = stage_input.a_inst_color;
        vert_main();
        SPIRV_Cross_Output stage_output;
        stage_output.gl_Position = gl_Position;
        stage_output.uv = uv;
        stage_output.color = color;
        return stage_output;
    }
*/
#if defined(SOKOL_D3D11)
static const uint8_t vs_source_hlsl5[1066] = {
    ,
};
#endif
/*
    Texture2D<float4> u_tex : register(t0);
    SamplerState u_smp : register(s1);

    static float4 frag_color;
    static float2 uv;
    static float4 color;

    struct SPIRV_Cross_Input
    {
        float4 color : TEXCOORD0;
        float2 uv : TEXCOORD1;
    };

    struct SPIRV_Cross_Output
    {
        float4 frag_color : SV_Target0;
    };

    void frag_main()
    {
        frag_color = u_tex.Sample(u_smp, uv) * color;
    }

    SPIRV_Cross_Output main(SPIRV_Cross_Input stage_input)
    {
        uv = stage_input.uv;
        color = stage_input.color;
        frag_main();
        SPIRV_Cross_Output stage_output;
        stage_output.frag_color = frag_color;
        return stage_output;
    }
*/
#if defined(SOKOL_D3D11)
static const uint8_t fs_source_hlsl5[607] = {
    ,
};
#endif
/*
    #include <metal_stdlib>
    #include <simd/simd.h>

    using namespace metal;

    struct vs_params
    {
        float4x4 u_mvp;
    };

    struct main0_out
    {
        float4 color [[user(locn0)]];
        float2 uv [[user(locn1)]];
        float4 gl_Position [[position]];
    };

    struct main0_in
    {
        float2 a_pos [[attribute(0)]];
        float2 a_uv [[attribute(1)]];
        float2 a_inst_pos [[attribute(2)]];
        float4 a_inst_color [[attribute(3)]];
    };

    vertex main0_out main0(main0_in in [[stage_in]], constant vs_params& _33 [[buffer(0)]])
    {
        main0_out out = {};
        out.gl_Position = _33.u_mvp * float4(in.a_pos + in.a_inst_pos, 0.0, 1.0);
        out.uv = in.a_uv;
        out.color = in.a_inst_color;
        return out;
    }

*/
#if defined(SOKOL_METAL)
static const uint8_t vs_source_metal_macos[680] = {
    ,
};
#endif
/*
    #include <metal_stdlib>
    #include <simd/simd.h>

    using namespace metal;

    struct main0_out
    {
        float4 frag_color [[color(0)]];
    };

    struct main0_in
    {
        float4 color [[user(locn0)]];
        float2 uv [[user(locn1)]];
    };

    fragment main0_out main0(main0_in in [[stage_in]], texture2d<float> u_tex [[texture(0)]], sampler u_smp [[sampler(1)]])
    {
        main0_out out = {};
        out.frag_color = u_tex.sample(u_smp, in.uv) * in.color;
        return out;
    }

*/
#if defined(SOKOL_METAL)
static const uint8_t fs_source_metal_macos[444] = {
    ,
};
#endif
/*
    #include <metal_stdlib>
    #include <simd/simd.h>

    using namespace metal;

    struct vs_params
    {
        float4x4 u_mvp;
    };

    struct main0_out
    {
        float4 color [[user(locn0)]];
        float2 uv [[user(locn1)]];
        float4 gl_Position [[position]];
    };

    struct main0_in
    {
        float2 a_pos [[attribute(0)]];
        float2 a_uv [[attribute(1)]];
        float2 a_inst_pos [[attribute(2)]];
        float4 a_inst_color [[attribute(3)]];
    };

    vertex main0_out main0(main0_in in [[stage_in]], constant vs_params& _33 [[buffer(0)]])
    {
        main0_out out = {};
        out.gl_Position = _33.u_mvp * float4(in.a_pos + in.a_inst_pos, 0.0, 1.0);
        out.uv = in.a_uv;
        out.color = in.a_inst_color;
        return out;
    }

*/
#if defined(SOKOL_METAL)
static const uint8_t vs_source_metal_ios[680] = {
    ,
};
#endif
/*
    #include <metal_stdlib>
    #include <simd/simd.h>

    using namespace metal;

    struct main0_out
    {
        float4 frag_color [[color(0)]];
    };

    struct main0_in
    {
        float4 color [[user(locn0)]];
        float2 uv [[user(locn1)]];
    };

    fragment main0_out main0(main0_in in [[stage_in]], texture2d<float> u_tex [[texture(0)]], sampler u_smp [[sampler(1)]])
    {
        main0_out out = {};
        out.frag_color = u_tex.sample(u_smp, in.uv) * in.color;
        return out;
    }

*/
#if defined(SOKOL_METAL)
static const uint8_t fs_source_metal_ios[444] = {
    ,
};
#endif
/*
    #include <metal_stdlib>
    #include <simd/simd.h>

    using namespace metal;

    struct vs_params
    {
        float4x4 u_mvp;
    };

    struct main0_out
    {
        float4 color [[user(locn0)]];
        float2 uv [[user(locn1)]];
        float4 gl_Position [[position]];
    };

    struct main0_in
    {
        float2 a_pos [[attribute(0)]];
        float2 a_uv [[attribute(1)]];
        float2 a_inst_pos [[attribute(2)]];
        float4 a_inst_color [[attribute(3)]];
    };

    vertex main0_out main0(main0_in in [[stage_in]], constant vs_params& _33 [[buffer(0)]])
    {
        main0_out out = {};
        out.gl_Position = _33.u_mvp * float4(in.a_pos + in.a_inst_pos, 0.0, 1.0);
        out.uv = in.a_uv;
        out.color = in.a_inst_color;
        return out;
    }

*/
#if defined(SOKOL_METAL)
static const uint8_t vs_source_metal_sim[680] = {
    ,
};
#endif
/*
    #include <metal_stdlib>
    #include <simd/simd.h>

    using namespace metal;

    struct main0_out
    {
        float4 frag_color [[color(0)]];
    };

    struct main0_in
    {
        float4 color [[user(locn0)]];
        float2 uv [[user(locn1)]];
    };

    fragment main0_out main0(main0_in in [[stage_in]], texture2d<float> u_tex [[texture(0)]], sampler u_smp [[sampler(1)]])
    {
        main0_out out = {};
        out.frag_color = u_tex.sample(u_smp, in.uv) * in.color;
        return out;
    }

*/
#if defined(SOKOL_METAL)
static const uint8_t fs_source_metal_sim[444] = {
    ,
};
#endif
const sg_shader_desc* particle_shader_desc(sg_backend backend) {
    #if defined(SOKOL_GLCORE)
    if (backend == SG_BACKEND_GLCORE) {
        static sg_shader_desc desc;
        static bool valid;
        if (!valid) {
            valid = true;
            desc.vertex_func.source = (const char*)vs_source_glsl410;
            desc.vertex_func.entry = "main";
            desc.fragment_func.source = (const char*)fs_source_glsl410;
            desc.fragment_func.entry = "main";
            desc.attrs[0].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[0].glsl_name = "a_pos";
            desc.attrs[1].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[1].glsl_name = "a_uv";
            desc.attrs[2].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[2].glsl_name = "a_inst_pos";
            desc.attrs[3].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[3].glsl_name = "a_inst_color";
            desc.uniform_blocks[0].stage = SG_SHADERSTAGE_VERTEX;
            desc.uniform_blocks[0].layout = SG_UNIFORMLAYOUT_STD140;
            desc.uniform_blocks[0].size = 64;
            desc.uniform_blocks[0].glsl_uniforms[0].type = SG_UNIFORMTYPE_FLOAT4;
            desc.uniform_blocks[0].glsl_uniforms[0].array_count = 4;
            desc.uniform_blocks[0].glsl_uniforms[0].glsl_name = "vs_params";
            desc.views[0].texture.stage = SG_SHADERSTAGE_FRAGMENT;
            desc.views[0].texture.image_type = SG_IMAGETYPE_2D;
            desc.views[0].texture.sample_type = SG_IMAGESAMPLETYPE_FLOAT;
            desc.views[0].texture.multisampled = false;
            desc.samplers[1].stage = SG_SHADERSTAGE_FRAGMENT;
            desc.samplers[1].sampler_type = SG_SAMPLERTYPE_FILTERING;
            desc.texture_sampler_pairs[0].stage = SG_SHADERSTAGE_FRAGMENT;
            desc.texture_sampler_pairs[0].view_slot = 0;
            desc.texture_sampler_pairs[0].sampler_slot = 1;
            desc.texture_sampler_pairs[0].glsl_name = "u_tex_u_smp";
            desc.label = "particle_shader";
        }
        return &desc;
    }
    #endif /* SOKOL_GLCORE */
    #if defined(SOKOL_GLES3)
    if (backend == SG_BACKEND_GLES3) {
        static sg_shader_desc desc;
        static bool valid;
        if (!valid) {
            valid = true;
            desc.vertex_func.source = (const char*)vs_source_glsl300es;
            desc.vertex_func.entry = "main";
            desc.fragment_func.source = (const char*)fs_source_glsl300es;
            desc.fragment_func.entry = "main";
            desc.attrs[0].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[0].glsl_name = "a_pos";
            desc.attrs[1].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[1].glsl_name = "a_uv";
            desc.attrs[2].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[2].glsl_name = "a_inst_pos";
            desc.attrs[3].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[3].glsl_name = "a_inst_color";
            desc.uniform_blocks[0].stage = SG_SHADERSTAGE_VERTEX;
            desc.uniform_blocks[0].layout = SG_UNIFORMLAYOUT_STD140;
            desc.uniform_blocks[0].size = 64;
            desc.uniform_blocks[0].glsl_uniforms[0].type = SG_UNIFORMTYPE_FLOAT4;
            desc.uniform_blocks[0].glsl_uniforms[0].array_count = 4;
            desc.uniform_blocks[0].glsl_uniforms[0].glsl_name = "vs_params";
            desc.views[0].texture.stage = SG_SHADERSTAGE_FRAGMENT;
            desc.views[0].texture.image_type = SG_IMAGETYPE_2D;
            desc.views[0].texture.sample_type = SG_IMAGESAMPLETYPE_FLOAT;
            desc.views[0].texture.multisampled = false;
            desc.samplers[1].stage = SG_SHADERSTAGE_FRAGMENT;
            desc.samplers[1].sampler_type = SG_SAMPLERTYPE_FILTERING;
            desc.texture_sampler_pairs[0].stage = SG_SHADERSTAGE_FRAGMENT;
            desc.texture_sampler_pairs[0].view_slot = 0;
            desc.texture_sampler_pairs[0].sampler_slot = 1;
            desc.texture_sampler_pairs[0].glsl_name = "u_tex_u_smp";
            desc.label = "particle_shader";
        }
        return &desc;
    }
    #endif /* SOKOL_GLES3 */
    #if defined(SOKOL_D3D11)
    if (backend == SG_BACKEND_D3D11) {
        static sg_shader_desc desc;
        static bool valid;
        if (!valid) {
            valid = true;
            desc.vertex_func.source = (const char*)vs_source_hlsl5;
            desc.vertex_func.d3d11_target = "vs_5_0";
            desc.vertex_func.entry = "main";
            desc.fragment_func.source = (const char*)fs_source_hlsl5;
            desc.fragment_func.d3d11_target = "ps_5_0";
            desc.fragment_func.entry = "main";
            desc.attrs[0].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[0].hlsl_sem_name = "TEXCOORD";
            desc.attrs[0].hlsl_sem_index = 0;
            desc.attrs[1].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[1].hlsl_sem_name = "TEXCOORD";
            desc.attrs[1].hlsl_sem_index = 1;
            desc.attrs[2].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[2].hlsl_sem_name = "TEXCOORD";
            desc.attrs[2].hlsl_sem_index = 2;
            desc.attrs[3].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[3].hlsl_sem_name = "TEXCOORD";
            desc.attrs[3].hlsl_sem_index = 3;
            desc.uniform_blocks[0].stage = SG_SHADERSTAGE_VERTEX;
            desc.uniform_blocks[0].layout = SG_UNIFORMLAYOUT_STD140;
            desc.uniform_blocks[0].size = 64;
            desc.uniform_blocks[0].hlsl_register_b_n = 0;
            desc.views[0].texture.stage = SG_SHADERSTAGE_FRAGMENT;
            desc.views[0].texture.image_type = SG_IMAGETYPE_2D;
            desc.views[0].texture.sample_type = SG_IMAGESAMPLETYPE_FLOAT;
            desc.views[0].texture.multisampled = false;
            desc.views[0].texture.hlsl_register_t_n = 0;
            desc.samplers[1].stage = SG_SHADERSTAGE_FRAGMENT;
            desc.samplers[1].sampler_type = SG_SAMPLERTYPE_FILTERING;
            desc.samplers[1].hlsl_register_s_n = 1;
            desc.texture_sampler_pairs[0].stage = SG_SHADERSTAGE_FRAGMENT;
            desc.texture_sampler_pairs[0].view_slot = 0;
            desc.texture_sampler_pairs[0].sampler_slot = 1;
            desc.label = "particle_shader";
        }
        return &desc;
    }
    #endif /* SOKOL_D3D11 */
    #if defined(SOKOL_METAL)
    if (backend == SG_BACKEND_METAL_MACOS) {
        static sg_shader_desc desc;
        static bool valid;
        if (!valid) {
            valid = true;
            desc.vertex_func.source = (const char*)vs_source_metal_macos;
            desc.vertex_func.entry = "main0";
            desc.fragment_func.source = (const char*)fs_source_metal_macos;
            desc.fragment_func.entry = "main0";
            desc.attrs[0].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[1].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[2].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[3].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.uniform_blocks[0].stage = SG_SHADERSTAGE_VERTEX;
            desc.uniform_blocks[0].layout = SG_UNIFORMLAYOUT_STD140;
            desc.uniform_blocks[0].size = 64;
            desc.uniform_blocks[0].msl_buffer_n = 0;
            desc.views[0].texture.stage = SG_SHADERSTAGE_FRAGMENT;
            desc.views[0].texture.image_type = SG_IMAGETYPE_2D;
            desc.views[0].texture.sample_type = SG_IMAGESAMPLETYPE_FLOAT;
            desc.views[0].texture.multisampled = false;
            desc.views[0].texture.msl_texture_n = 0;
            desc.samplers[1].stage = SG_SHADERSTAGE_FRAGMENT;
            desc.samplers[1].sampler_type = SG_SAMPLERTYPE_FILTERING;
            desc.samplers[1].msl_sampler_n = 1;
            desc.texture_sampler_pairs[0].stage = SG_SHADERSTAGE_FRAGMENT;
            desc.texture_sampler_pairs[0].view_slot = 0;
            desc.texture_sampler_pairs[0].sampler_slot = 1;
            desc.label = "particle_shader";
        }
        return &desc;
    }
    #endif /* SOKOL_METAL */
    #if defined(SOKOL_METAL)
    if (backend == SG_BACKEND_METAL_IOS) {
        static sg_shader_desc desc;
        static bool valid;
        if (!valid) {
            valid = true;
            desc.vertex_func.source = (const char*)vs_source_metal_ios;
            desc.vertex_func.entry = "main0";
            desc.fragment_func.source = (const char*)fs_source_metal_ios;
            desc.fragment_func.entry = "main0";
            desc.attrs[0].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[1].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[2].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[3].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.uniform_blocks[0].stage = SG_SHADERSTAGE_VERTEX;
            desc.uniform_blocks[0].layout = SG_UNIFORMLAYOUT_STD140;
            desc.uniform_blocks[0].size = 64;
            desc.uniform_blocks[0].msl_buffer_n = 0;
            desc.views[0].texture.stage = SG_SHADERSTAGE_FRAGMENT;
            desc.views[0].texture.image_type = SG_IMAGETYPE_2D;
            desc.views[0].texture.sample_type = SG_IMAGESAMPLETYPE_FLOAT;
            desc.views[0].texture.multisampled = false;
            desc.views[0].texture.msl_texture_n = 0;
            desc.samplers[1].stage = SG_SHADERSTAGE_FRAGMENT;
            desc.samplers[1].sampler_type = SG_SAMPLERTYPE_FILTERING;
            desc.samplers[1].msl_sampler_n = 1;
            desc.texture_sampler_pairs[0].stage = SG_SHADERSTAGE_FRAGMENT;
            desc.texture_sampler_pairs[0].view_slot = 0;
            desc.texture_sampler_pairs[0].sampler_slot = 1;
            desc.label = "particle_shader";
        }
        return &desc;
    }
    #endif /* SOKOL_METAL */
    #if defined(SOKOL_METAL)
    if (backend == SG_BACKEND_METAL_SIMULATOR) {
        static sg_shader_desc desc;
        static bool valid;
        if (!valid) {
            valid = true;
            desc.vertex_func.source = (const char*)vs_source_metal_sim;
            desc.vertex_func.entry = "main0";
            desc.fragment_func.source = (const char*)fs_source_metal_sim;
            desc.fragment_func.entry = "main0";
            desc.attrs[0].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[1].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[2].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.attrs[3].base_type = SG_SHADERATTRBASETYPE_FLOAT;
            desc.uniform_blocks[0].stage = SG_SHADERSTAGE_VERTEX;
            desc.uniform_blocks[0].layout = SG_UNIFORMLAYOUT_STD140;
            desc.uniform_blocks[0].size = 64;
            desc.uniform_blocks[0].msl_buffer_n = 0;
            desc.views[0].texture.stage = SG_SHADERSTAGE_FRAGMENT;
            desc.views[0].texture.image_type = SG_IMAGETYPE_2D;
            desc.views[0].texture.sample_type = SG_IMAGESAMPLETYPE_FLOAT;
            desc.views[0].texture.multisampled = false;
            desc.views[0].texture.msl_texture_n = 0;
            desc.samplers[1].stage = SG_SHADERSTAGE_FRAGMENT;
            desc.samplers[1].sampler_type = SG_SAMPLERTYPE_FILTERING;
            desc.samplers[1].msl_sampler_n = 1;
            desc.texture_sampler_pairs[0].stage = SG_SHADERSTAGE_FRAGMENT;
            desc.texture_sampler_pairs[0].view_slot = 0;
            desc.texture_sampler_pairs[0].sampler_slot = 1;
            desc.label = "particle_shader";
        }
        return &desc;
    }
    #endif /* SOKOL_METAL */
    return 0;
}
int particle_attr_slot(const char* attr_name) {
    (void)attr_name;
    if (0 == strcmp(attr_name, "a_pos")) {
        return 0;
    }
    if (0 == strcmp(attr_name, "a_uv")) {
        return 1;
    }
    if (0 == strcmp(attr_name, "a_inst_pos")) {
        return 2;
    }
    if (0 == strcmp(attr_name, "a_inst_color")) {
        return 3;
    }
    return -1;
}
int particle_texture_slot(const char* tex_name) {
    (void)tex_name;
    if (0 == strcmp(tex_name, "u_tex")) {
        return 0;
    }
    return -1;
}
int particle_sampler_slot(const char* smp_name) {
    (void)smp_name;
    if (0 == strcmp(smp_name, "u_smp")) {
        return 1;
    }
    return -1;
}
int particle_uniformblock_slot(const char* ub_name) {
    (void)ub_name;
    if (0 == strcmp(ub_name, "vs_params")) {
        return 0;
    }
    return -1;
}
size_t particle_uniformblock_size(const char* ub_name) {
    (void)ub_name;
    if (0 == strcmp(ub_name, "vs_params")) {
        return sizeof(vs_params_t);
    }
    return 0;
}
int particle_uniform_offset(const char* ub_name, const char* u_name) {
    (void)ub_name; (void)u_name;
    if (0 == strcmp(ub_name, "vs_params")) {
        if (0 == strcmp(u_name, "u_mvp")) {
            return 0;
        }
    }
    return -1;
}
sg_glsl_shader_uniform particle_uniform_desc(const char* ub_name, const char* u_name) {
    (void)ub_name; (void)u_name;
    #if defined(__cplusplus)
    sg_glsl_shader_uniform res = {};
    #else
    sg_glsl_shader_uniform res = {0};
    #endif
    if (0 == strcmp(ub_name, "vs_params")) {
        if (0 == strcmp(u_name, "u_mvp")) {
            res.type = SG_UNIFORMTYPE_MAT4;
            res.array_count = 0;
            res.glsl_name = "u_mvp";
            return res;
        }
    }
    return res;
}
int particle_storagebuffer_slot(const char* sbuf_name) {
    (void)sbuf_name;
    return -1;
}
int particle_storageimage_slot(const char* simg_name) {
    (void)simg_name;
    return -1;
}
#endif // SOKOL_SHDC_IMPL